...
always @(posedge sys_clk)begin          //根据键盘的输入对使能输出赋值
    if(uart_rx_data==49)begin            //如果按下键盘1，由于串口传输的是ASCII码，1对应49，所以此时uart_rx_data==49，则使能信号变为1
        en=1;
    end
    else if(uart_rx_data==50)begin       //如果按下键盘2，由于串口传输的是ASCII码，1对应49，所以此时uart_rx_data==50，则使能信号变为0
        en=0;
    end
